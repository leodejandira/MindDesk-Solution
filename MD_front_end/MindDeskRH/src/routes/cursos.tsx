    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold">Arquivos {isAdmin ? "enviados" : "disponíveis"}</h3>
...
function CursosTab({
  ...
  useEffect(() => {
    fetchCursos();
    if (isAdmin) fetchFuncionarios();
  }, [isAdmin]);

  const fetchCursos = async () => {
    setLoading(true);
    try {
      const endpoint = isAdmin ? `${API}/cursos/todos` : `${API}/cursos`;
